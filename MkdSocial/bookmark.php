...
<?php 
            $sql = mysqli_query($conn, "SELECT * FROM bookmark WHERE bookmark_member_id = {$_SESSION['unique_id']}");
            if(mysqli_num_rows($sql) > 0){
              $row2 = mysqli_fetch_assoc($sql);
            }
            ?>
<?php            
								if ( isset($row2['bookmark_member_id']) )
									{ ?>
  ...
  <div class="container">
        <div class="row">
        <?php            
              $query = (mysqli_query($conn, "SELECT * FROM bookmark INNER JOIN post ON post.post_id=bookmark.bookmark_post_id
                                                                    INNER JOIN users ON post.post_member_id=users.unique_id
			  													 WHERE bookmark_member_id={$_SESSION['unique_id']} ORDER BY post.created_date DESC "));
